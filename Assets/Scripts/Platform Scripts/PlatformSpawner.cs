...
public class PlatformSpawner : MonoBehaviour {
  ...
  void Start() { current_Platform_Spawn_Timer = platform_Spawn_Timer; }

  // Update is called once per frame
  void Update() { SpawnPlatforms(); }
      ...
      if (platform_Spawn_Count < 2) {
        ...
        if (Random.Range(0, 2) > 0) {
          ...
          newPlatform = Instantiate(
              moving_Platforms[Random.Range(0, moving_Platforms.Length)], temp,
              Quaternion.identity);
        }
        ...
        if (Random.Range(0, 2) > 0) {
          // generate regular platform
          newPlatform =
              Instantiate(platform_Prefab, temp,
                          Quaternion.identity); // same as line 47, 56 and 92 -
                                                // repetetive code - not DRY
        } else {
          // or new spike platform
          newPlatform =
              Instantiate(spike_Platform_Prefab, temp, Quaternion.identity);
        }
        ...
        if (Random.Range(0, 2) > 0) {
          ...
          // or new spike platform
          newPlatform =
              Instantiate(breakable_Platform, temp, Quaternion.identity);
        }
